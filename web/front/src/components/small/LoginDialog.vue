  ...
  methods: {
    sendEmail: function () {
      if (this.form.username === '') {
        ...
        $.ajax({
          type: 'GET',
          url: 'http://localhost:80/Demo_war_exploded/Hello/send_email/' + this.form.username,
          crossDomain: true,
          xhrFields: {
    ...
    submit_again (encrypted) { // 加密后再次把密码发给后端
      ...
      $.ajax({
        type: 'POST',
        url: 'http://localhost:80/Demo_war_exploded/Hello/login_check',
        crossDomain: true,
        xhrFields: {
          withCredentials: true
        },
    ...
    submit: function (formName) {
      ...
      if (canSubmit) {
        ...
        $.ajax({
          type: 'GET',
          url: 'http://localhost:80/Demo_war_exploded/Hello/login_request',
          crossDomain: true,
          xhrFields: {
            withCredentials: true
          },
